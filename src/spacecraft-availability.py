...
def main():
    ...
    with spice_client.KernelPool():
        for key, info in spacecraft_info.items():
            ...
            coord = SkyCoord(
                CartesianRepresentation(info["Positions"].T),
                obstime=info["Time"],
                frame="icrs",
            ).transform_to(frames.HeliographicStonyhurst)

            info["Heliolongitude"] = coord.lon
            info["Heliolatitude"] = coord.lat
    ...
    # Contruct our plot
    _, axes = plt.subplots(
        4, 1, height_ratios=[1, 2, 2, 1], figsize=(6, 6), sharex=True
    )

    sunspot_ax, longitude_ax, latitude_ax, ax = axes

    # Plot sunspot number onto sunspot_ax
    sunspot_time = [decimal_year_to_datetime(t) for t in sunspots["Decimal Year"]]
    sunspot_ax.plot(sunspot_time, sunspots["Mean"], color="black")

    sunspot_ax.set_ylabel("Mean Monthly\nSunspot Number")

    # Plot lon and lat for each spacecraft
    for i, (_, info) in enumerate(spacecraft_info.items()):

        longitude_ax.plot(info["Time"], info["Heliolongitude"])
        latitude_ax.plot(info["Time"], info["Heliolatitude"])

    longitude_ax.set_ylabel("Heliographic\nLongitude [$^\circ$]")
    latitude_ax.set_ylabel("Heliographic\nLatitude [$^\circ$]")

    # Plot the availability on ax
    for i, (_, info) in enumerate(spacecraft_info.items()):
    ...
    ax.margins(x=0, y=0)
    ax.set_xlim(dt.datetime(1975, 1, 1), ax.get_xlim()[1])
    ax.set_ylim(-0.5, len(spacecraft_info) - 0.5)
    ax.tick_params(axis="x", rotation=-90)
    ax.xaxis.set_minor_locator(MonthLocator(interval=12))
...
def decimal_year_to_datetime(decimal_year: float) -> dt.datetime:
